...
def config():
    os.environ["JINA_DATA_FILE"] = os.environ.get(
        "JINA_DATA_FILE", "data/monster-description.csv"
    )
...
def print_topk(resp, sentence):
    for d in resp.search.docs:
        ...
        for idx, match in enumerate(d.matches):
            ...
            if score < 0.0:
                continue
            monster = match.meta_info.decode()
            description = match.text.strip()
            print(f'> {idx:>2d}({score:.2f}). {monster.upper()} is described as "{description}"')
...
def query(top_k):
    ...
    with f:
        while True:
            text = input("please type a description: ")
            if not text:
                break
